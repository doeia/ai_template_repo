...
def remove_background(input_path: str, display_output: bool = False):
    """
    ...
    # Extract the mask out of the processes frame
    ...
    # Create a white background having the same dimension as the original frame
    background_frame = np.zeros(
        [frame.shape[0], frame.shape[1], frame.shape[2]], dtype=np.uint8)
    background_frame.fill(255)
